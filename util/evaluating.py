...
class Metrics(object):
    ...
    def cal_precision(self):
        ...
        for tag in self.tagset:
            if self.predict_tags_counter.get(tag) is None:
                continue
    ...
    def cal_recall(self):
        ...
        recall_scores = {}
        for tag in self.tagset:
            if self.golden_tags_counter.get(tag) is None:
                continue
    ...
    def cal_f1(self):
        f1_scores = {}
        for tag in self.tagset:
            if self.precision_scores.get(tag) is None:
                continue
    ...
    def report_scores(self):
        ...
        for tag in self.tagset:
            print(row_format.format(
                tag,
                self.precision_scores.get(tag, -0.1),
                self.recall_scores.get(tag, -0.1),
                self.f1_scores.get(tag, -0.1),
                self.golden_tags_counter.get(tag, -0.1),
            ))
